...
float PhSensor::levelOutPhLevel(float phLevel) {
    ...
    logger.log(LogLevel::INFO, "First Unsuccessful Reading:", firstReading);
    ...
    if (increase) {
        ...
        return increasePh(clampedTime);
    } else {
        // call method for decreasing
        return decreasePh(clampedTime);
    }    
}

float PhSensor::increasePh(float timeToChange) {
...
float PhSensor::decreasePh(float timeToChange) {
    logger.log(LogLevel::INFO, "How long to rotate pump (new):", timeToChange, "\n");    
    ...
    // keeping track of what's happened
    timeLastRotatedFor = timeToChange;
    logger.log(LogLevel::INFO, "Needs to decrease the ph levels, not set up.", "\n");
    return 0;
}
